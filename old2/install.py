...
class BoxButton(urwid.WidgetWrap):
    ...
    def keypress(self, size, key):
        ...
        self._emit('click', self._user_data)
...
class ArchLinuxInstaller(urwid.Frame):
    ...
    def __init__(self):
        ...
        self.installBtn = BoxButton('Install', user_data=True)
        self.cancelBtn = BoxButton('Cancel', user_data=False)
        self.partitionLabel = []
        self.gptRadioBtn = urwid.RadioButton(self.partitionLabel, 'GPT Partition')
        self.mbrRadioBtn = urwid.RadioButton(self.partitionLabel, 'MBR Partition')
        ...
        super().__init__(
                            ...
                            (35, urwid.Pile(
                                widget_list=[
                                    ...
                                    urwid.LineBox(
                                        ...
                                        title='Partition Chooser'
                                    ),
                                ]
                            )),
                            urwid.LineBox(
                                original_widget=self.details,
                                title='Details'
                            ),
                        ]
                    ),
            ...
            header=urwid.LineBox(urwid.AttrMap(self.instructions, 'help')),
            footer=urwid.Columns(
                widget_list=[
                    urwid.AttrMap(urwid.Text('\nUse tab to arrive here -->>>\n', urwid.CENTER), 'help'),
                    (10, self.cancelBtn),
                    (10, self.installBtn)
                ],
                dividechars=5,
        ...
        urwid.connect_signal(self.cancelBtn, 'click', self.exit)
        urwid.connect_signal(self.installBtn, 'click', self.exit)
        urwid.connect_signal(self.gptRadioBtn, 'change', self.selectPartitionLabel, 'GPT')
        urwid.connect_signal(self.mbrRadioBtn, 'change', self.selectPartitionLabel, 'MBR')

    def exit(self, item, data):
        self.result['userAccepted'] = data
        raise urwid.ExitMainLoop()

    def selectPartitionLabel(self, item, state, data):
        if state:
            self.result['partitionLabel'] = data

    def set_state(self, state):
        if state['partitionLabel'] is not None:
            self.gptRadioBtn.set_state('GPT' == state['partitionLabel'])
            self.mbrRadioBtn.set_state('MBR' == state['partitionLabel'])

    def updateInformation(self):
        if self.gptRadioBtn in self.get_focus_widgets() or self.mbrRadioBtn in self.get_focus_widgets():
            self.instructions.set_text('Use arrow UP/DOWN to move; Use SPACE/ENTER/CLICK to select')
        elif self.installBtn in self.get_focus_widgets() or self.cancelBtn in self.get_focus_widgets():
            self.instructions.set_text('Use arrow LEFT/RIGHT to move; Use SPACE/ENTER/CLICK to select')
        else:
...
# 'light magenta', 'light cyan', 'white'
    ...
    view = ArchLinuxInstaller()

    main = urwid.MainLoop(view, palette=palette, unhandled_input=exit_on_q)

    main.run()

    print(json.dumps(view.result, sort_keys=True, indent=2))